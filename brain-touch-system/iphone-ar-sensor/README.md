...
この段階では、STLサンプル頂点との近似距離だけをデバッグ表示しています。まだ本番用の三角形最近傍探索やBVHは入れていないため、正式なタッチ判定は既存の楕円体判定のままです。まず「アプリ内でSTLが読める」「STLサイズとscaleが画面で確認できる」「保存済みキャリブレーション値でSTL投影を重ねられる」「指先とSTL表面の距離感を見られる」状態を作っています。
