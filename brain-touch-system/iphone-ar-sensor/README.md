...
周囲に椅子や固定具が映る環境でも、それらを置いたまま `Capture Empty Baseline` を実行してください。現在の抽出は同じピクセルの空状態との差だけを使うため、動いていない周辺物体は脳候補に入りません。空撮影後にiPhoneが約2.5cmまたは約2度以上動いた場合は誤った位置を保存せず、再撮影を促します。脳の推定は5フレームの中央値で確定します。

黄色bboxと緑の点群が脳から椅子側へ伸びる場合は、椅子を取り除くのではなく、椅子を最終配置に固定した状態で空撮影からやり直します。空撮影後はiPhone、椅子、固定具を動かさず、脳模型だけを追加してください。
